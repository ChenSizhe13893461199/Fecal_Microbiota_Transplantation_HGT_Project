...
for i in "${!pre_samples[@]}"; do
    ...
    python HGT.py \
        ...
        # (non-HGT) regions covers at least 70% of the full contig length if n = 0.7; 
        ...
        # guaranteeing sufficient genomic context for reliable alignment and minimizing false positives caused by isolated HGT-like segments embedded within non-homologous regions
    mkdir -p HGT
...
# For each HGT‑associated contig, calculates the GC percentage of the sequences outside the merged HGT intervals.
# Appends two new columns to the existing {post}_HGT_statistics.txt file: GC_nonHGT (recipient non‑HGT GC) and GC_nonHGTdonor (donor non‑HGT GC).
